...
assert.match(main, /launchOptions\.firstRun \|\| launchOptions\.background[\s\S]{0,80}\? await pendingStart[\s\S]{0,80}: await serviceRuntime\.waitUntilListening\(0\)/, 'foreground startup must follow authoritative local readiness instead of opening Recovery on a shorter UI-only timeout');
