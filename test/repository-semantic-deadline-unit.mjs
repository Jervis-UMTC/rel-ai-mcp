...
try {
  ...
  assert.equal(recoveredStatus.metadata?.zoekt?.available, true,
    'cold semantic search must leave the optional Zoekt index available for deferred refresh');
  assert.equal(
    recoveredStatus.zoektRefreshScheduled || recoveredStatus.active || recoveredStatus.metadata?.zoekt?.current === true,
    true,
    'a deferred cold Zoekt rebuild must be pending, running, or complete after graph readiness'
  );
} finally {
