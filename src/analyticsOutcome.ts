...
import {
  ANALYTICS_OUTCOME_CLASSES,
  type AnalyticsOutcome,
  type ObservabilityResultInput,
  type ReliabilityCounters
} from './telemetry.types.ts';

const OUTCOME_CLASSES = ANALYTICS_OUTCOME_CLASSES;

function classifyAnalyticsOutcome(event: ObservabilityResultInput = {}): AnalyticsOutcome {
  ...
  if (operation === 'relai_validate' || operation === OP.VALIDATE_CHECKS || operation === OP.VALIDATE_DIAGNOSTICS || operation === OP.VALIDATE_HTTP) {
