...
import type { NativeTaskStatus } from '../contracts/tasks.ts';
import { getStateDir } from '../statePaths.js';
import { isSqliteBusyError, setStateMeta, stateDatabasePath, stateMetaValue, withStateDatabase } from '../stateDatabase.ts';
import { NATIVE_TASK_TRANSITIONS, canTransitionNativeTaskStatus, isTerminalNativeTaskStatus, normalizeNativeTaskStatus } from '../taskState.js';
import { normalizePrincipalKey, principalFingerprint } from './principal.js';

const TASK_TRANSITIONS = NATIVE_TASK_TRANSITIONS;
...
type NativeTaskNowSource = number | (() => number) | undefined;
type NativeTaskRestartPolicy = 'resumable' | 'restart_reconcilable' | 'non_resumable';

interface NativeTaskConfig extends Record<string, unknown> {
  stateDir?: string;
}

interface NativeTaskAccessOptions {
  now?: NativeTaskNowSource;
  principal?: Principal | undefined;
  logicalTaskId?: string | undefined;
}

interface NativeTaskMutationOptions extends NativeTaskAccessOptions {
  statusMessage?: string | undefined;
  cancelledStatusMessage?: string | undefined;
  executionStopped?: boolean | undefined;
}

interface NativeTaskExecutorInput {
  controller?: AbortController | undefined;
  resume?: ((responses: Record<string, unknown>) => unknown | Promise<unknown>) | undefined;
}

interface NativeTaskCreateOptions extends NativeTaskMutationOptions {
  status?: NativeTaskStatus | undefined;
  ...
  restartPolicy?: NativeTaskRestartPolicy | undefined;
  recovery?: unknown;
  internal?: Record<string, unknown> | undefined;
  executor?: NativeTaskExecutorInput | undefined;
}

interface NativeTaskUpdatePatch {
  status?: NativeTaskStatus | undefined;
  statusMessage?: string | undefined;
  pollIntervalMs?: number | undefined;
  ttlMs?: number | null | undefined;
...
interface NativeTaskDto extends Record<string, unknown> {
  taskId: string;
...
interface NativeTaskExecutorState {
  controller: AbortController;
  resume: ((responses: Record<string, unknown>) => unknown | Promise<unknown>) | null;
  resumeInFlight: boolean;
  rawInputResponses: Record<string, unknown>;
}

const executors = new Map<string, NativeTaskExecutorState>();
const lastPruneAtByDirectory = new Map<string, number>();
const activeNativeDatabases = new Map<string, any>();
const migratedNativeDatabases = new Set<string>();
const inputSchemaValidator = new Ajv({ allErrors: true, allowUnionTypes: true, strict: false });
const inputSchemaValidators = new Map<string, any>();
const MAX_INPUT_SCHEMA_VALIDATORS = 128;

class NativeTaskUnavailableError extends Error {
  readonly code = 'NATIVE_TASK_UNAVAILABLE';
...
class NativeTaskRequestError extends Error {
  readonly code = 'NATIVE_TASK_INVALID_REQUEST';
  readonly reason: string;

  constructor(message: any, reason: any = 'invalid_request') {
...
class NativeTaskStoreError extends Error {
  readonly code = 'NATIVE_TASK_STORE_ERROR';
  readonly reason: string;
  readonly retryable: boolean;
  readonly taskId: string;

  constructor(reason: any, options: any = {}) {
...
function createNativeTask(config: NativeTaskConfig, options: NativeTaskCreateOptions = {}): NativeTaskDto {
...
function getNativeTask(config: NativeTaskConfig, taskId: unknown, options: NativeTaskAccessOptions = {}): NativeTaskDto {
...
function getNativeTaskRecord(config: NativeTaskConfig, taskId: unknown, options: NativeTaskAccessOptions = {}): Record<string, any> {
  return withTaskLock(config, taskId, () => {
    const task = requireTaskUnlocked(config, taskId, options);
    reconcileTaskUnlocked(config, task, options.now);
    return clone(task);
...
function updateNativeTask(config: NativeTaskConfig, taskId: unknown, patch: NativeTaskUpdatePatch = {}, options: NativeTaskAccessOptions = {}): NativeTaskDto {
  ...
  const unknown = Object.keys(patch).filter((key: any) => !allowed.has(key));
...
function updateNativeTaskRecovery(config: NativeTaskConfig, taskId: unknown, recovery: unknown, options: NativeTaskAccessOptions = {}): NativeTaskDto {
  return withTaskLock(config, taskId, () => {
    const task = requireTaskUnlocked(config, taskId, options);
    reconcileTaskUnlocked(config, task, options.now);
    assertActiveTask(task);
...
function completeNativeTask(config: NativeTaskConfig, taskId: unknown, result: unknown, options: NativeTaskMutationOptions = {}): NativeTaskDto {
...
function failNativeTask(config: NativeTaskConfig, taskId: unknown, error: unknown, options: NativeTaskMutationOptions = {}): NativeTaskDto {
  return transitionTerminal(config, taskId, 'failed', {
...
function requestNativeTaskInput(config: NativeTaskConfig, taskId: unknown, inputRequests: unknown, options: NativeTaskMutationOptions = {}): NativeTaskDto {
...
function updateNativeTaskInputs(config: NativeTaskConfig, taskId: unknown, inputResponses: unknown, options: NativeTaskMutationOptions = {}): NativeTaskDto {
  ...
  let acceptedRaw: Record<string, unknown> = {};
...
function cancelNativeTask(config: NativeTaskConfig, taskId: unknown, options: NativeTaskMutationOptions = {}): NativeTaskDto {
...
function acknowledgeNativeTaskCancellation(config: NativeTaskConfig, taskId: unknown, options: NativeTaskMutationOptions = {}): NativeTaskDto {
  return withTaskLock(config, taskId, () => {
    const task = requireTaskUnlocked(config, taskId, options);
    if (task.status === 'cancelled') return detailedTask(task);
...
function attachNativeTaskExecutor(taskId: unknown, executor: NativeTaskExecutorInput = {}): AbortSignal | undefined {
  const id = validateTaskId(taskId);
  attachExecutor(id, executor);
  return executors.get(id)?.controller.signal;
}

function nativeTaskSignal(taskId: unknown): AbortSignal | undefined {
  return executors.get(validateTaskId(taskId))?.controller?.signal;
}

function pruneNativeTasks(config: any, options: any = {}) {
  ...
  let quarantined = 0;
  try {
    withStateDatabase(config, (db: any) => withActiveNativeDatabase(config, db, () => {
...
function opportunisticPrune(config: any, nowMs: any) {
...
function pruneNativeTaskQuarantine(config: any, nowSource: any) {
  migrateLegacyNativeTasks(config);
  const cutoff = nowValue(nowSource) - QUARANTINE_RETENTION_MS;
  try {
    return withStateDatabase(config, (db: any) => {
...
function transitionTerminal(config: any, taskId: any, status: any, payload: any, options: any = {}) {
  return withTaskLock(config, taskId, () => {
    const task = requireTaskUnlocked(config, taskId, options);
    if (isTerminalNativeTaskStatus(task.status)) {
...
function sameTerminalOutcome(task: any, status: any, payload: any) {
...
function terminalConflict(current: any, requested: any) {
...
function requireTaskUnlocked(config: any, taskId: any, options: any = {}) {
  const id = validateTaskId(taskId);
...
function attachExecutor(taskId: string, executor: NativeTaskExecutorInput) {
...
async function retryNativeTaskOperation<TResult>(
  ...
  while (true) {
    try {
      ...
      await new Promise((resolve: any) => setTimeout(resolve, Math.min(delayMs, Math.max(1, deadline - Date.now()))));
    }
  }
}

function nativeDatabaseKey(config: any) {
  return stateDatabasePath(config);
}

function currentNativeDatabase(config: any) {
  return activeNativeDatabases.get(nativeDatabaseKey(config)) || null;
}

function withActiveNativeDatabase(config: any, db: any, operation: any) {
...
function migrateLegacyNativeTasks(config: any = {}) {
  const key = nativeDatabaseKey(config);
  ...
  try {
    withStateDatabase(config, (db: any) => {
      if (stateMetaValue(db, LEGACY_NATIVE_MIGRATION_KEY, '') === '1') return;
      const importDirectory = (directory: any, quarantineOnly: any = false) => {
        let entries;
        try {
...
function withTaskLock(config: any, taskId: any, operation: any) {
  const id = validateTaskId(taskId);
  migrateLegacyNativeTasks(config);
  try {
    return withStateDatabase(
      config,
      (db: any) => withActiveNativeDatabase(config, db, operation),
      { transaction: true, timeoutMs: 0 }
    );
  } catch (error: any) {
...
function readTask(config: any, taskId: any) {
  const id = validateTaskId(taskId);
  migrateLegacyNativeTasks(config);
  const read = (db: any) => {
...
function normalizeStoredTask(parsed: any, taskId: any) {
  ...
  const satisfiedInputKeys = Array.isArray(parsed.satisfiedInputKeys)
    ? [...new Set(parsed.satisfiedInputKeys.map((key: any) => validateInputKey(key)))].slice(0, MAX_INPUT_ENTRIES)
    : [];
  const inputUpdates = Array.isArray(parsed.inputUpdates)
    ? parsed.inputUpdates.slice(-MAX_INPUT_UPDATES).map((entry: any, index: any) => normalizeInputUpdate(entry, index + 1))
    : [];
  if (status === 'input_required' && Object.keys(inputRequests).length === 0) {
    throw taskStoreError('record_corrupt', null, taskId);
  }
...
function persistTask(config: any, task: any) {
  ...
  try {
    withNativeWriteDatabase(config, (db: any) => {
      db.prepare(`INSERT INTO native_tasks(task_id,updated_at_ms,payload) VALUES(?,?,?)
        ON CONFLICT(task_id) DO UPDATE SET updated_at_ms=excluded.updated_at_ms,payload=excluded.payload`)
...
function removeTask(config: any, taskId: any) {
  const id = validateTaskId(taskId);
  migrateLegacyNativeTasks(config);
...
function quarantineTaskRecord(config: any, taskId: any, nowSource: any = undefined) {
  const id = validateTaskId(taskId);
  migrateLegacyNativeTasks(config);
  try {
    const quarantined = withNativeWriteDatabase(config, (db: any) => {
      const row = db.prepare('SELECT payload FROM native_tasks WHERE task_id=?').get(id);
      if (!row) return false;
      db.prepare(`INSERT INTO native_task_quarantine(task_id,quarantined_at_ms,reason,payload)
...
function abortExecutor(taskId: any, reason: any) {
...
function taskStoreError(reason: any, cause: any, taskId: any = '') {
  return new NativeTaskStoreError(reason, { cause: cause || undefined, taskId });
}

function detailedTask(task: any): NativeTaskDto {
  const result: NativeTaskDto = {
    taskId: task.taskId,
    status: task.status,
...
function safeEqual(left: any, right: any) {
  const a = Buffer.from(String(left || ''));
  const b = Buffer.from(String(right || ''));
  return a.length === b.length && crypto.timingSafeEqual(a, b);
}

function normalizeStatus(value: unknown): NativeTaskStatus {
  const raw = String(value || '').toLowerCase();
  const status = normalizeNativeTaskStatus(raw);
  if (!status) throw new NativeTaskRequestError(`Invalid native task status: ${raw}`);
  return status;
}

function assertTaskTransition(current: unknown, next: unknown): NativeTaskStatus {
  const from = normalizeStatus(current);
  const to = normalizeStatus(next);
  if (!canTransitionNativeTaskStatus(from, to)) {
    throw new NativeTaskRequestError(`Invalid native task transition: ${from} -> ${to}`, 'invalid_transition');
  }
  return to;
}

function assertActiveTask(task: any) {
  if (isTerminalNativeTaskStatus(task.status)) throw terminalConflict(task.status, task.status);
}

function normalizeRestartPolicy(value: any) {
...
function normalizeTtl(value: any) {
...
function normalizePollInterval(value: any) {
...
function normalizeInputRequests(value: any) {
...
function sanitizeInputRequest(descriptor: any) {
...
function normalizeInputResponses(value: any) {
...
function normalizeInputObject(value: any, label: any) {
...
function validateInputKey(value: any) {
  const key = String(value || '');
  if (!INPUT_KEY_PATTERN.test(key)) throw new NativeTaskRequestError('Task input key is invalid.');
  return key;
}

function normalizeInputUpdate(entry: any, fallbackSequence: any) {
...
function validateInputResponse(key: any, request: any, value: any) {
  const schema = request?.responseSchema ?? request?.schema;
  if (schema === undefined || schema === null || schema === true) return;
  const label = `Input response ${key}`;
  const cacheKey = stableJson(schema);
  let validate = inputSchemaValidators.get(cacheKey);
  if (!validate) {
    try {
      validate = inputSchemaValidator.compile(schema);
    } catch (error: any) {
      throw new NativeTaskRequestError(`${label} schema is invalid: ${error instanceof Error ? error.message : String(error)}`);
    }
    inputSchemaValidators.set(cacheKey, validate);
    if (inputSchemaValidators.size > MAX_INPUT_SCHEMA_VALIDATORS) {
      const oldestKey = inputSchemaValidators.keys().next().value;
      if (oldestKey !== undefined) inputSchemaValidators.delete(oldestKey);
    }
  }
  if (validate(value)) return;
  const details = inputSchemaValidator.errorsText(validate.errors, { separator: '; ' });
  throw new NativeTaskRequestError(`${label} does not satisfy its schema${details ? `: ${details}` : '.'}`);
}

function normalizeJsonRpcError(error: any) {
...
function normalizeBoundedJson(value: any, maxBytes: any, label: any, options: any = {}) {
  let serialized;
  try {
...
function redactSensitiveJson(value: any, keyHint: any = ''): any {
  if (/authorization|cookie|password|passphrase|private.?key|secret|token/i.test(keyHint)) return '[redacted]';
  if (Array.isArray(value)) return value.map((item: any) => redactSensitiveJson(item));
...
function sanitizeErrorMessage(value: any) {
  return boundedMessage(value)
    .replace(/\bBearer\s+[A-Za-z0-9._~+/=-]+/gi, 'Bearer [redacted]')
    .replace(/\b(?:token|secret|password|authorization)\s*[:=]\s*[^\s,;]+/gi, (match: any) => `${match.split(/[:=]/, 1)[0]}=[redacted]`)
    .replace(/[A-Za-z]:\\(?:[^\s\\]+\\)*[^\s]*/g, '[path]')
    .replace(/\/(?:home|Users)\/[^\s]+/g, '[path]');
}

function boundedMessage(value: any) {
...
function boundedText(value: any, maxChars: any) {
  return boundedMessage(value).slice(0, maxChars);
}

function normalizeOptionalTimestamp(value: any) {
...
function touchTask(task: any, nowSource: any) {
  task.lastUpdatedAt = new Date(nowValue(nowSource)).toISOString();
}

function isExpired(task: any, nowSource: any) {
