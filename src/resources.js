...
Use \`relai_edit\` as the single file mutation tool. Destructive operations may return \`input_required\`; retry with the accepted response and integrity-protected requestState. Native asynchronous work is returned only when the current request advertises \`io.modelcontextprotocol/tasks\`, then polled with \`tasks/get\` and controlled with \`tasks/update\` or \`tasks/cancel\`. Without that capability, eligible long work may continue under \`work_id\`: keep doing useful independent work instead of polling, consume any later \`completedOperations\` notice, and call \`relai_work\` status only when the background result is needed.
