import { setSpanAttributes } from './telemetry.ts';
import { updateCurrentToolActivity } from './toolActivity.js';
import { sanitizeDisplayText } from './taskObservability.js';
import type { ExecutionPlanMetrics } from './telemetry.types.ts';

interface ExecutionPlanObserverEvent {
  type?: 'step_started' | 'step_completed' | string;
  name?: string;
  total?: unknown;
  completed?: unknown;
  metadata?: { displayName?: unknown } | null;
}

interface ExecutionPlanObserverOptions {
...
function createExecutionPlanObserver(options: ExecutionPlanObserverOptions = {}): (event?: ExecutionPlanObserverEvent) => void {
  const running = new Map<string, string>();
  const update = typeof options.update === 'function'
    ? options.update
    : updateCurrentToolActivity as (patch: Record<string, unknown>) => unknown;
  ...
  return (event: ExecutionPlanObserverEvent = {}) => {
    const name = event.name || '';
    if (event.type === 'step_started') running.set(name, displayName(event));
    if (event.type === 'step_completed') running.delete(name);
...
function executionMetricAttributes(kind: unknown, metrics: ExecutionPlanMetrics = {}): Record<string, string | number> {
...
function recordExecutionPlanMetrics(kind: unknown, metrics: ExecutionPlanMetrics = {}): void {
  setSpanAttributes(executionMetricAttributes(kind, metrics));
}

function displayName(event: ExecutionPlanObserverEvent): string {
