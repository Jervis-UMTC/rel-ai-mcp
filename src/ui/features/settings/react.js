...
function ApplicationUpdates({ lifecycle }) {
  ...
  return h(Card, { title: 'App updates', className: 'application-update-panel' },
    h(ToggleRow, {
      ...
      help: 'Downloads a verified update in the background when one is found. Rel.AI still asks before restarting or opening the macOS installer.',
...
function updateView(status = {}, autoDownload = false) {
  ...
  if (state === 'downloaded') { const opensDmg = status.installMode === 'open_dmg'; return { label: 'Ready to install', tone: 'ok', description: opensDmg ? `${availableVersion || 'The update'} is verified and downloaded. Open the DMG, then replace Rel.AI MCP in Applications.` : `${availableVersion || 'The update'} is ready. Restart when Rel.AI is not working on a task.`, action: { id: 'install', label: opensDmg ? 'DMG' : 'Restart and install', className: 'primary' } }; }
