...
const OPERATION_DEFINITION_VALUES = [
  ...
  },
  {
    ...
    description: "Control the user-authorized local desktop through structured Windows UI Automation when available and Midscene pixel input as the fallback. Coordinates are local to the selected display; when displayId is omitted, Midscene targets the primary display. Status is available while control is disabled; every other action requires the explicit Computer control setting. Pass app for per-app approval (one approve_app per session); browsers are view-only so use relai_browser, terminals and IDEs are click-only so use relai_desktop for typing. Prefer observe for native apps: it returns bounded semantic controls and semanticObservationId; activate revalidates a target then clicks its current display-local center through Midscene. If semantic observation is unavailable, observe falls back to an adaptive screenshot. Screenshots return short-lived observationId values; balanced capture is the default, and coordinate actions may pass observationId to map screenshot pixels back to physical display pixels. Use wait_for_change inside batch(actions) to replace model-side polling and fuse deterministic activate/click/type/wait/screenshot sequences in one call. stop releases the single-session lock (Esc/abort).",
    inputSchema: {"type":"object","properties":{"workspace":{"type":"string"},"action":{"type":"string","enum":["status","displays","observe","activate","screenshot","wait_for_change","move","click","double_click","right_click","drag","scroll","type","key","hotkey","batch","stop","approve_app","revoke_app"]},"displayId":{"type":"string","minLength":1,"maxLength":200},"app":{"type":"string","minLength":1,"maxLength":200},"x":{"type":"number","minimum":0,"maximum":100000},"y":{"type":"number","minimum":0,"maximum":100000},"toX":{"type":"number","minimum":0,"maximum":100000},"toY":{"type":"number","minimum":0,"maximum":100000},"direction":{"type":"string","enum":["up","down","left","right"]},"distance":{"type":"number","minimum":1,"maximum":100000},"text":{"type":"string","minLength":1,"maxLength":65536},"key":{"type":"string","minLength":1,"maxLength":100},"keys":{"type":"array","items":{"type":"string","minLength":1,"maxLength":100},"minItems":2,"maxItems":8},"actions":{"type":"array","items":{"type":"object"},"minItems":1,"maxItems":20},"observationId":{"type":"string","minLength":1,"maxLength":100},"previousObservationId":{"type":"string","minLength":1,"maxLength":100},"profile":{"type":"string","enum":["fast","balanced","detail"]},"forceImage":{"type":"boolean"},"timeoutMs":{"type":"number","minimum":100,"maximum":30000},"pollMs":{"type":"number","minimum":50,"maximum":1000},"semanticObservationId":{"type":"string","minLength":1,"maxLength":100},"targetId":{"type":"string","minLength":1,"maxLength":100},"maxElements":{"type":"number","minimum":1,"maximum":300}},"required":["workspace","action"],"additionalProperties":false},
