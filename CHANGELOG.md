...
### Agent-managed learning and continuity
- **Remove Rel.AI's duplicate generic Saved Memory feature.** Settings now exposes focused agent learning instead of a second fact/preference memory store; the dashboard memory form/API, FTS-backed generic memory rows, and memory bootstrap injection are removed. Existing legacy memory tables are dropped by knowledge schema v4, while same-conversation/cross-project task continuity, validated path-to-command affinity, and explicit agent-managed `relai_skill` learning remain intact.
- **Replace automatic procedure inference with explicit agent-authored skills.** Rel.AI now exposes `relai_skill` create, edit, patch, and delete actions so ChatGPT decides when a demonstrated workflow is worth saving; create/edit/patch remain tied to current task evidence, source-changing work must have current passed validation, and deletion can use the authorized workspace directly without inventing an unrelated task.
- **Keep learned skills local, scoped, and provenance-owned.** Agent-authored skills live in Rel.AI-managed storage, default to project scope, participate in normal skill discovery and bootstrap suggestions, validate their `SKILL.md` frontmatter/body before write, and refuse to overwrite directories Rel.AI does not own.
...
- **Focus Settings on agent learning instead of duplicate memory.** The Learning page now shows only Rel.AI-managed learned skills, supports deleting a managed skill by scope, and clears learned validation affinity plus managed learning without touching unrelated user/project skills.
...
- **Support per-action task scope in the canonical schema/runtime.** Mixed tools such as `relai_ui`, `relai_changes`, and `relai_skill` now derive `work_id` requirements from the selected action instead of inheriting one unnecessarily strict tool-wide requirement.
...
- **Make agent-managed skill writes crash-recoverable.** `SKILL.md` and `PROVENANCE.json` are now assembled together in a staged directory, an existing complete skill is retained as a backup during replacement, and discovery/write paths recover the previous complete directory and discard incomplete pending state after an interrupted swap.
...
- **Reposition Rel.AI as a ChatGPT Web harness rather than a plain MCP bridge.** Public documentation now makes the ownership split explicit: ChatGPT remains the model, conversation, and reasoning host while Rel.AI owns the authorized local agency/runtime layer for repositories, work sessions, validation, Git, processes, agent-managed skills/learning, observability, desktop lifecycle, and opt-in computer control; MCP is described as the primary tool transport rather than the product identity.
...
- **Compact model-facing discovery without removing tool context.** Action ownership and action-specific bounds are now summarized once on each action selector instead of repeating `Action usage` prose on individual fields, reducing the 14-tool discovery payload from about 38.5 KB to about 34.7 KB while retaining every callable field, useful field guidance, and runtime validation.
...
- **Advance the connector surface to 14 model-facing tools at tool-surface version 68.** `relai_skill` and the explicit opt-in `relai_computer` surface are both part of the canonical manifest, per-action scope remains synchronized across executable/public schemas and UI invocation labels, and release metadata records the final 14-tool contract for this release.
